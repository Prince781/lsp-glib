...
/**
 * The editor, from the server's perspective. Exposes methods and notifications
 * that should be called from server code. If you want to ''implement'' a LSP
 * editor/client, use {@link Lsp.Client}.
 * 
 * This is a wrapper over {@link Jsonrpc.Client} that supports LSP-specific operations.
 */
public class Lsp.Editor : Object {
    Lsp.Server server;
    Jsonrpc.Client client;

    internal Editor (Server server, Jsonrpc.Client client) {
        this.server = server;
        this.client = client;
    }
